...
class TuttoGustoSupabaseUser extends BaseAuthUser {
  TuttoGustoSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> tuttoGustoSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = TuttoGustoSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
